...
public class oldc {
    ...
    public oldc(Pedina[][] board, PrintWriter out) {
        this.board = board;
        this.out = out;
    }
            ...
            for (int j = 0; j < MAX; j++) {
                ...
                cells[i][j].addMouseListener(new MouseAdapter() {
                    ...
                    public void mouseClicked(MouseEvent e) {
                        ...
                        // Cerca un percorso per muoverti
                        ...
                        // Qui dentro ho il percorso da seguire: allPath[index];

                        ArrayList<Pedina> piecesEaten = new ArrayList<>();

                        for (int i = 0; i < pathChosen.size(); i++) {
                            System.out.println("Pezzo mangiato: ");
                            piecesEaten.add(pathChosen.get(i).pieceEaten);
                        }

                        for (Pedina piece : piecesEaten) {
                            if (piece != null) {
                                System.out.println("Devo rimuovere la pedina:::" + piece.getPosizione());
                                removePedina(getPedinaFromPosition(
                                        new Posizione(piece.getPosizione().getX(), piece.getPosizione().getY())));
                            }
                        }

                        System.out.println(pathChosen);

                        Node lastElement = pathChosen.get(pathChosen.size() - 1);

                        if (allPath.size() > 0)
                            movePiece(oldRow, oldCol, lastElement.y, lastElement.x, true, pathChosen);
                    }
                });
    ...
    public void removePedina11(Pedina pedina) {

        System.out.println("Devi rimuovere la pedina!!!");
        int x = pedina.getPosizione().getX();
        int y = pedina.getPosizione().getY();

        System.out.println("x: " + x + ";y:" + y);

        for (int i = 0; i < allPedineGrafiche.size(); ++i) {
            if (allPedineGrafiche.get(i).getX() == x && allPedineGrafiche.get(i).getY() == y) {
                allPedineGrafiche.remove(allPedineGrafiche.get(i));
    ...
    public void movePiece(int oldRow, int oldCol, int row, int col, Boolean callServer, ArrayList<Node> pathChosen) {
        ...
        if (pathChosen != null)
            System.out.println(pathChosen.toString());

        // Comunica il movimento al server
        if (callServer)
            out.println("movePiece#" + oldCol + "-" + oldRow + "#" + col + "-" + row + "#" + pathChosen);
    }
